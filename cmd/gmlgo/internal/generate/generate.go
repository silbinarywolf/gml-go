...
const (
	...
	version       = "0.1.0"
	genUidComment = `// GmlUid is generated by "gmlgo generate"
	...
	//
	{
		...
		if len(structsUsingGMLObject) > 0 {
			for _, record := range structsUsingGMLObject {
				...
				newAssetID := assetidgen.NewID()

				// make sure formatted output is syntactically correct
				res := buf.String()
				structDefPattern := "type " + record.Name + " struct"
				replaceWith := fmt.Sprintf(genUidComment+"\nfunc (*"+record.Name+") GmlUid() uint64 {\n\treturn %d\n}\n\n%s", newAssetID, structDefPattern)
				newRes := strings.Replace(res, structDefPattern, replaceWith, 1)
				if res == newRes {
					panic("Unable to find pattern to replace \"" + structDefPattern + "\"")
				}
				...
				//panic(string(newRes) + "\n" + fmt.Sprintf("%s", record.FileSet.File(record.File.Pos()).Name()))
			}
		}
	}
