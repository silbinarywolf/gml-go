...
const (
	...
	version        = "0.2.0"
	objectIdPrefix = "Obj"
	genUidComment  = `// %s is generated by "gmlgo generate". DO NOT EDIT.
...
type Struct struct {
	...
	Struct *types.Struct
	ID     string
	// GmlUid is the non-zero unique ID of the struct.
	// This is used to tie objects to other kinds of data
	GmlUid int

	// Node is used to give error information (column/line) for the
	// struct.
	Node *ast.TypeSpec
	// FileSet is used to give error information (column/line) for the
	// struct.
	FileSet *token.FileSet
	File    *ast.File
}
...
func inspectGameObjectStructs(pkg *Package) []Struct {
	...
	for _, file := range pkg.files {
		...
		ast.Inspect(file.file, func(node ast.Node) bool {
			...
			case *ast.GenDecl:
				if n.Tok == token.TYPE {
					// NOTE(Jae): 2020-03-29
					// we need this so that we fallthrough to the
					// *ast.TypeSpec case
					return true
				}
				...
				spec, ok := n.Specs[0].(*ast.ValueSpec)
				if !ok {
					return false
				}
				if len(spec.Names) != 1 &&
				...
				uid, err := strconv.Atoi(lit.Value)
				if err != nil {
					return false
				}
				typeInfo, ok := spec.Type.(*ast.SelectorExpr)
				if !ok {
					return false
				}
				namespace, ok := typeInfo.X.(*ast.Ident)
				if !ok {
					return false
				}
				// TODO(Jae): 2020-03-29
				// Make this lookup the scope of "gml" to confirm
				// its the correct package so we can support aliasing
				if namespace.Name != "gml" {
					return false
				}
				if typeInfo.Sel.Name != "ObjectIndex" {
					return false
				}
				constantName := spec.Names[0].Name
				if !strings.HasPrefix(constantName, objectIdPrefix) {
					return false
				}
				structName := strings.Replace(constantName, objectIdPrefix, "", 1)
				structToUid[structName] = uid
			// type XXXX struct
			case *ast.TypeSpec:
	...
	for i := 0; i < len(structsUsingGMLObject); i++ {
		...
		if uid, ok := structToUid[s.Name]; ok {
			s.GmlUid = uid
			delete(structToUid, s.Name)
		}
	}
	if len(structToUid) > 0 {
		// NOTE(Jae): 2020-03-29
		// Consider making this detect unused/invalid UIDs and auto-remove them?
	}
...
func generateGameObject(dir string, packageName string, structsUsingGMLObject []Struct) {
	...
	// check for duplicates and provide an error message
	{
	...
	if len(structsUsingGMLObject) > 0 {
		for _, record := range structsUsingGMLObject {
			...
			if err := cfg.Fprint(&buf, record.FileSet, record.File); err != nil {
				panic(fmt.Errorf("Unexpected failure while trying to update unique id for struct: %s", err))
			}

			newCode := fmt.Sprintf("const %s%s gml.ObjectIndex = %d", objectIdPrefix, record.Name, assetidgen.NewID())

			res := buf.String()
			structDefPattern := "type " + record.Name + " struct"
			newRes := strings.Replace(res, structDefPattern, fmt.Sprintf(genUidComment, objectIdPrefix+record.Name)+"\n"+newCode+"\n\n"+structDefPattern, 1)
			if res == newRes {
				panic("Unexpected error. Unable to find pattern to replace \"" + structDefPattern + "\"")
			}
			// Format
			src, err := format.Source([]byte(newRes))
			if err != nil {
				// Should never happen, but can arise when developing this code.
				// The user can compile the output to see the error.
				log.Fatalf("invalid Go generated: %s\n%s", err, buf.Bytes())
			}
			filename := record.FileSet.File(record.File.Pos()).Name()
	...
	if len(structsUsingGMLObject) > 0 {
		// Generate object meta
		g.Printf("func init() {\n")
		for _, record := range structsUsingGMLObject {
			assetName := record.Name
			g.Printf("\tgml.UnsafeAddObjectGeneratedData(" + objectIdPrefix + assetName + ", new(" + assetName + "))\n")
		}
		g.Printf("}\n\n")
